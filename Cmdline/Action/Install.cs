        ...
        public int RunCommand(CKAN.GameInstance inst, object args)
        {
            ...
            for (var done = false; !done;)
            {
                ...
                try
                {
                    ...
                    var installer = ModuleInstaller.GetInstance(inst, _manager.Cache, _user);
                ...
                catch (DependencyNotSatisfiedKraken kraken)
                {
                    ...
                    {
                        _user.RaiseMessage("\"{0}\" requires \"{1}\" but it is not listed in the index, or not available for your version of {2}.", kraken.parent, kraken.module, inst.game.ShortName);
                    }
                    else
                    {
                        _user.RaiseMessage("\"{0}\" requires \"{1}\" {2} but it is not listed in the index, or not available for your version of {3}.", kraken.parent, kraken.module, kraken.version, inst.game.ShortName);
                    }
                ...
                catch (ModuleNotFoundKraken kraken)
                {
                    if (kraken.version == null)
                    {
                        _user.RaiseMessage("The mod \"{0}\" is required but it is not listed in the index, or not available for your version of {1}.", kraken.module, inst.game.ShortName);
                    }
                    else
                    {
                        _user.RaiseMessage("The mod \"{0}\" {1} is required but it is not listed in the index, or not available for your version of {2}.", kraken.module, kraken.version, inst.game.ShortName);
                    }
                ...
                {
                    _user.RaiseMessage("Bad metadata detected for mod {0}.\r\n{1}", kraken.module, kraken.Message);
                    return Exit.Error;
                }
                ...
                catch (InconsistentKraken kraken)
                {
                    ...
                    _user.RaiseMessage(kraken.InconsistenciesPretty);
                    _user.RaiseMessage("Install canceled. Your files have been returned to their initial state.");
                    return Exit.Error;
                }
                catch (CancelledActionKraken)
                {
                    _user.RaiseMessage("Installation canceled at user request.");
                    return Exit.Error;
                }
                ...
                {
                    _user.RaiseMessage("One or more files failed to download, stopped.");
                    return Exit.Error;
                }
                ...
                {
                    _user.RaiseMessage("Can't install the expansion \"{0}\".", kraken.module.name);
